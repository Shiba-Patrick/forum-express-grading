...
const userController = {
  ...
  editUser: (req, res, next) => { // edit頁面瀏覽
  ...
  addFavorite: (req, res, next) => { // 加入我的最愛
    const { restaurantId } = req.params
    // 先確認錯誤資料流入,先用兩組查詢來防呆:如果...不存在或以已存在就結束執行
    return Promise.all([
  ...
  removeFavorite: (req, res, next) => { // 加入我的最愛 移除我的最愛
  ...
  removeLike: (req, res, next) => { // 收回Like
    return Like.findOne({
      where: {
        userId: req.user.id,
        restaurantId: req.params.restaurantId
  ...
  addFollowing: (req, res, next) => { // 增加追蹤者
  ...
  removeFollowing: (req, res, next) => { // 移除追蹤者
    Followship.findOne({
      where: {
        followerId: req.user.id,
        followingId: req.params.userId
